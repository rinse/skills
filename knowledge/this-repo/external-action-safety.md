...
- `gh-pr-create` / `gh-issue-create`: 担当者・マイルストーンは希望時のみ。ラベルは内容（差分／課題）に合うものを既存ラベルから**自動選定**するが、付けすぎず、**存在しないラベルは勝手に作らずユーザーに確認**する。
- **AI 作成ラベル**（`AI ✨`）: `gh-pr-create` / `gh-issue-create` は AI 作成を示すラベルが既にあれば付与する。無ければ作成可否をユーザーに尋ね、**断られたらリポジトリ単位で記憶**して次回以降は尋ねない（記憶はリポジトリ外の Memory 機構に限り、そのためにファイル作成・作業ツリー変更はしない）。意味マッチは保守的に寄せ（`bot` / `automated` 等の曖昧なラベルは対象外）、ユーザーに質問できない非対話環境（autopilot 等）では作成・質問を省略する。
