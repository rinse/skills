...
差分が非常に大きい場合、差分の読解（ファイルごとの変更概要の把握、疑わしい箇所の列挙といった事実の抽出）だけを Agent ツールで安価な Subagent（`model: "haiku"` 等）に委譲してよい。ただし**指摘の採否や severity の判断は必ずレビュアー自身が行う**（gh-release スキルと同様、判断は手元に残す）。
