...
更新する PR を一つに確定させます。**誤った PR を編集すると取り消しづらく、関係者にも見えてしまう**ため、対象は次のいずれかで**明確になっている場合に限り**進めます。

- **ユーザーが id（PR 番号）や URL を共有している。**
- **文脈から対象が一意に明らか。** 例: いまのセッションで作成・議論したばかりの PR、現在のブランチに紐づく PR がちょうど一つだけでその PR の話をしている、など。

どちらにも当てはまらず**対象が一意に定まらない場合は、推測で進めず、どの PR か（番号・URL）をユーザーに尋ねます。** 候補が複数あるとき・現在ブランチに PR が無いとき・会話の対象がはっきりしないときは、必ず確認します。

対象を `gh pr view <num-or-url> --json number,title,url,headRefName,baseRefName` で引き、**取得できたタイトル・ブランチが意図した PR と一致するか**を確かめてから次へ進みます（人違いの PR を掴んでいないかの最終チェック）。
...
`gh pr diff` は **origin に push 済みの内容**しか反映しません。ローカルに未 push のコミットがある場合、GitHub 側の差分は古いままです。ローカルの差分を正とみなして本文を書きつつ、**未 push がある旨を後述の確認・後処理で扱います**（push は後処理で行う）。
...
- 一時的に作った本文ファイルは片付ける。
- **未 push のローカルコミットがある場合**は、コードの変更を GitHub に反映するために push する。**まず一度 `git push origin <branch>` を試す**（push 先は明示する）。成功すればそれで完了。
- **push が失敗した場合**（認証・パスフレーズ入力が対話的に必要、権限が無い等）は、無理に繰り返さず、その時点で**ユーザーに push を依頼する**。ユーザー自身のシェルなら対話的な入力もできる。失敗の内容を簡潔に伝え、実行してほしいコマンド（`git push origin <branch>`）を示す。

## フォールバック: GitHub MCP

`gh` が使えず GitHub MCP が利用できる環境では、GitHub MCP の PR 編集ツールで同じことを行います。その場合も「現在の差分から title・body・labels を**合成し直す**」「古い記述を残さない」「編集前の確認」「push はまず試し、失敗したらユーザーに依頼」という方針は変わりません。現在のメタ情報・差分の取得、ラベルの増減も MCP の対応ツールで行います。
